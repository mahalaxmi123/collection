...
/**
 * Renderers to align Moodle's HTML with that expected by Elastic
 *
 * @package    theme_elastic
...
function elastic_grid($hassidepre, $hassidepost) {
...
function theme_elastic_page_init(moodle_page $page) {
...
function get_category_courses(){
	...
	$formcontent = '';
	$formcontent .= '<div class="category-wrapper">';
		foreach($categories as $category){
			$formcontent .= '<div class="category-list">';
			$formcontent .= '<div class="category-name"><a href="'.$CFG->wwwroot.'/course/index.php?categoryid='.$category->id.'">'.$category->name.'<span class="pull-right glyphicon glyphicon-chevron-right"></span></a></div>';
		$courses = $DB->get_records('course',array('category' => $category->id));
		$formcontent .= '<ul category-items>';
		foreach($courses as $course){
			   $formcontent .= '<li class="category-items-list"><a href="'.$CFG->wwwroot.'/course/view.php?id='.$course->id.'">'.$course->fullname.'</a></li>';
			}
			$formcontent .= '</ul>';
			$formcontent .= '</div>';
		}
